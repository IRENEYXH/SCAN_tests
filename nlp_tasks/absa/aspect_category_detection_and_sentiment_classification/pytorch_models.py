...
class GATLayer(nn.Module):
    ...
    def edge_attention(self, edges: dgl.udf.EdgeBatch):
...
class DglGraphConvolution(nn.Module):
    ...
    def _sum(self, nodes: dgl.udf.NodeBatch):
        """Take an average over all neighbor node features hu and use it to
        overwrite the original node feature."""
        m = nodes.mailbox['m']
        accum = torch.sum(m, 1)
    ...
    def _mean(self, nodes: dgl.udf.NodeBatch):
        """Take an average over all neighbor node features hu and use it to
        overwrite the original node feature."""
        m = nodes.mailbox['m']
        mean = torch.mean(m, 1)
...
class DglGraphAttentionForAspectCategory(nn.Module):
    ...
    def _message_func(self, edges: dgl.udf.EdgeBatch):
    ...
    def _attention(self, nodes: dgl.udf.NodeBatch):
        """Take an average over all neighbor node features hu and use it to
        overwrite the original node feature."""
        m = nodes.mailbox['m']
        alphas = []
